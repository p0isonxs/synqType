...
export function useOptimizedBettingContract(roomId, enabled = true) {
    ...
    const callThrottle = useCallback((key, minInterval = 1000) => {
        const now = Date.now();
    ...
    const createRoomAndBet = useCallback(async (betAmount, timeLimit) => {
        if (!address)
    ...
    const joinRoom = useCallback(async (amount) => {
        if (!address)
            throw new Error('Wallet not connected');
    ...
    const declareGameResult = useCallback(async (players, scores) => {
    ...
    const roomData = useMemo(() => {
        ...
        return {
            host: roomInfo[0],
            betAmount: formatEther(roomInfo[1]),
            playerCount: Number(roomInfo[2]),
            gameEnded: roomInfo[3],
            winner: roomInfo[4],
            totalPot: formatEther(roomInfo[5]),
            timeLimit: Number(roomInfo[6]),
            createdAt: Number(roomInfo[7]),
        };
    }, [roomInfo]);
    const isHost = useMemo(() => address && roomData?.host?.toLowerCase() === address.toLowerCase(), [address, roomData?.host]);
    const isPlayer = hasJoined ?? false;
    const isWinner = useMemo(() => roomData?.winner?.toLowerCase() === address?.toLowerCase(), [roomData?.winner, address]);
    const canJoin = useMemo(() => !roomData?.gameEnded && !isPlayer, [roomData?.gameEnded, isPlayer]);
    const canDeclareFinished = useMemo(() => isPlayer && !roomData?.gameEnded && !hasFinished && canStart, [isPlayer, roomData?.gameEnded, hasFinished, canStart]);
    const gameCanStart = useMemo(() => canStart && roomData && roomData.playerCount >= 2, [canStart, roomData]);
    return {
        // State
        roomData,
        isHost,
