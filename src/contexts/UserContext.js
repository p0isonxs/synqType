...
export const UserProvider = ({ children }) => {
    ...
    const updateUserData = (updates) => {
        setUserData(prev => {
            ...
            if (updates.initials)
                localStorage.setItem("initials", updates.initials);
            if (updates.avatarUrl)
                localStorage.setItem("avatarUrl", updates.avatarUrl);
            if (updates.soundEnabled !== undefined)
                localStorage.setItem("soundEnabled", String(updates.soundEnabled));
            if (updates.roomSettings)
                localStorage.setItem("roomSettings", JSON.stringify(updates.roomSettings));
            if (updates.walletAddress)
                localStorage.setItem("walletAddress", updates.walletAddress);
            if (updates.walletConnected !== undefined)
                localStorage.setItem("walletConnected", String(updates.walletConnected)); // ✅ NEW
            return newData;
        });
    };
