...
export class PlayerModel extends ReactModel {
    ...
    lastAvatarTime = 0;
    lastInitialsTime = 0;
    updateThrottleMs = 100; // Max 10 updates per second
    walletThrottleMs = 5000; // Max once per 5 seconds
    avatarThrottleMs = 3000; // Max once per 3 seconds
    initialsThrottleMs = 2000; // Max once per 2 seconds
    init({ viewId, parent }) {
    ...
    typedWord(correct) {
        ...
        if (correct) {
            ...
            if (newIndex >= this.typingModel.words.length) {
                ...
                this.index = newIndex;
                this.progress = Math.min((this.score / this.typingModel.words.length) * 100, 100);
            }
        }
        this.updateWPM();
        // ✅ BATCH UPDATE: Only publish if significant changes
        const significantChange = this.score !== oldScore ||
    ...
    cachedRank = 0;
    lastRankCalculation = 0;
    rankCacheMs = 500; // Cache rank for 500ms
    getRank() {
        const now = this.now();
